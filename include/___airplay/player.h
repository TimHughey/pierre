...
#define time_ping_history                                                      \
  (1                                                                           \
   << time_ping_history_power_of_two) // 2^7 is 128. At 1 per three seconds,
                                      // approximately six minutes of records
...
// 512 is the minimum for normal operation -- it gives 512*352/44100 or just
// over 4 seconds of buffers. For at least 10 seconds, you need to go to 2048.
// Resend requests will be spaced out evenly in the latency period, subject to a
// minimum interval of about 0.25 seconds. Each buffer occupies 352*4 bytes plus
// about, say, 64 bytes of overhead in various places, say roughly 1,500 bytes
// per buffer. Thus, 2048 buffers will occupy about 3 megabytes -- no big deal
// in a normal machine but maybe a problem in an embedded device.
...
typedef struct {
  ...
  char client_ip_string[INET6_ADDRSTRLEN]; // the ip string pointing to the
                                           // client
  uint16_t client_rtsp_port;
  char self_ip_string[INET6_ADDRSTRLEN]; // the ip string being used by this
                                         // program -- it
  ...
  SOCKADDR rtp_client_control_socket; // a socket pointing to the control port
                                      // of the client
  SOCKADDR rtp_client_timing_socket;  // a socket pointing to the timing port of
                                      // the client
  ...
  char *airplay_gid; // UUID in the Bonjour advertisement -- if NULL, the group
  ...
  flush_request_t
      *flush_requests; // if non-null, there are flush requests, mutex protected
  ...
  int event_socket;
  SOCKADDR ap2_remote_control_socket_addr; // a socket pointing to the control
                                           // port of the client
  ...
  double
      local_to_remote_time_gradient; // if no drift, this would be exactly 1.0;
                                     // likely it's slightly above or  below.
  int local_to_remote_time_gradient_sample_count; // the number of samples used
                                                  // to calculate the gradient
  // add the following to the local time to get the remote time modulo 2^64
  uint64_t local_to_remote_time_difference; // used to switch between local and
                                            // remote clocks
  uint64_t local_to_remote_time_difference_measurement_time; // when the above
                                                             // was calculated
  ...
  int32_t buffer_occupancy; // allow it to be negative because seq_diff may be
                            // negative
  ...
  void *dapo_private_storage; // this is used for compatibility, if dacp stuff
                              // isn't enabled.
...
extern int statistics_row; // will be reset to zero when debug level changes or
                           // statistics enabled
...
// which is about 2*10^8 * 1,000 seconds at 384,000 frames per second -- about 2
// trillion seconds. assumes, without checking, that successive timestamps in a
// series always span an interval of less than one minute.
