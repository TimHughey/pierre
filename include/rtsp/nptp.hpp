...
public:
  ~Nptp();
  [[nodiscard]] static sNptp create(const string &app_name, sHost host) {
    // not using std::make_shared; constructor is private
    return sNptp(new Nptp(app_name, host));
  }
...
private:
  Nptp(const string &app_name, sHost host);
...
private:
  ...
  // previous list. If the master clock of the new list is the same as that of the old
  // list, the master clock is retained without resynchronisation; this means that
  // non-master devices can be added and removed without disturbing the SMI's existing
  // master clock. If no timing list is provided, the existing timing list is deleted. (In
  // future version of NQPTP the SMI interface may also be deleted at this point.) SMI
