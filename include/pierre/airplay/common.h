...
typedef enum {
  ST_basic =
      0,   // straight deletion or insertion of a frame in a 352-frame packet
...
#define debug_mutex_lock(mu, t, d)                                             \
  _debug_mutex_lock(mu, t, #mu, __FILE__, __LINE__, d)

int _debug_mutex_unlock(pthread_mutex_t *mutex, const char *mutexName,
                        const char *filename, const int line, int debuglevel);

#define debug_mutex_unlock(mu, d)                                              \
  _debug_mutex_unlock(mu, #mu, __FILE__, __LINE__, d)
