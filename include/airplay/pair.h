 ...
 * non-transient client pair setup, you also get a key string
 (client_setup_keys) from
 ...
 *
 --------------------------------|-------------------------------|--------------
 *  PAIR_CLIENT_FRUIT               | client keys                   | shared
 secret
 *  PAIR_CLIENT_HOMEKIT_NORMAL      | client keys, server public    | shared
 secret | key, server id                | shared secret
 *  PAIR_CLIENT_HOMEKIT_TRANSIENT   | shared secret                 | n/a
 *  PAIR_SERVER_HOMEKIT (normal)    | client public key, client id  | shared
 secret
...
typedef int (*pair_cb)(uint8_t public_key[32], const char *device_id,
                       void *cb_arg);
...
int pair_verify(uint8_t **out, size_t *out_len,
                struct pair_verify_context *sctx, const uint8_t *in,
                size_t in_len);
...
/* These are for constructing specific message types and reading specific
 * message types. Not needed for Homekit pairing where you can use
 * pair_verify().
 */
