...
FairPlay::FairPlay(RequestShared request) : Reply(request) {}
...
void FairPlay::payload1(uint8_t mode) {
  auto where = _payload.end();

  // fmt::print("{:>20} mode={:02d}\n", "creating payload1", mode);

  switch (mode) {
  case 0:
    _payload.insert(where, reply1, reply1 + std::size(reply1));
    break;

  case 1:
    _payload.insert(where, reply2, reply2 + std::size(reply2));
    break;

  case 2:
    _payload.insert(where, reply3, reply3 + std::size(reply3));
    break;

  case 3:
    _payload.insert(where, reply4, reply4 + std::size(reply4));
    break;
