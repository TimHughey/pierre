...
void Event::doAccept(yield_context yield) {
  do {
...
void Event::recvEvent(tcp_socket &socket, yield_context yield) {
  std::array<uint8_t, 4096> packet{0};

  mutable_buffer buf(packet.data(), packet.size());

  // fmt::print("Event::recvEvent(): accepted conn socket={}\n", socket.native_handle());
  ...
  socket.async_receive(buf, 0,
                       ...
                       [this, &packet, &socket, yield](const error_code &ec, size_t bytesp) {
                         if (ec != system::errc::success) {
                           ...
                           fmt::print("Event::recvEvent(): socket shutdown: {}\n", msg);
                         ...
                         if (type == time_announce) {
                           fmt::print("Event::recvEvent() Time Announce RTP packet ");
                           fmt::print("type={:#02x} bytes={}\n", type, bytesp);
                         } else {
                           //  fmt::print("Event::recvEvent() Unknown RTP packet ");
                           //  fmt::print("type={:#02x} bytes={}\n", type, bytesp);
                         }
