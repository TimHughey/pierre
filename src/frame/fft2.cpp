...
fftw_plan FFT2::plan{nullptr};
reals_t FFT2::wwf;

void FFT2::init() noexcept {

  if (plan == nullptr) {
    plan = fftw_plan_dft_r2c_1d(samples_max, in, out, FFTW_MEASURE);
  }
  ...
  if (wwf.size() == 0) {
    ...
    for (size_t i = 0; i < (samples_max >> 1); i++) {
      ...
      wwf[i] = with_compensation ? (weighingFactor * compensationFactor) : weighingFactor;
    }
  }
}
